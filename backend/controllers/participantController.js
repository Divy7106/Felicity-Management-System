...
// Updates participant interests and followed organizers during post-signup onboarding
...
// Fetches top 3 organizers ranked by total registration count across their events
...
// Returns all registrations for the current participant with enriched event data
...
// Searches and filters published events with fuzzy search, preference scoring, and sorting
...
// Returns top 5 trending events based on registration count in the last 24 hours
export const getTrendingEvents = async (req, res) => {
    try {
        const now = new Date()
        const last24h = new Date(Date.now() - 24 * 60 * 60 * 1000)

        let trending = await Registration.aggregate([
            { $match: { createdAt: { $gte: last24h }, status: { $ne: "cancelled" } } },
            { $group: { _id: "$eventId", count: { $sum: 1 } } },
            { $sort: { count: -1 } },
            { $limit: 5 }
        ])

        // Fallback: if no registrations in last 24h, get all-time top events
        if (trending.length === 0) {
            trending = await Registration.aggregate([
...
// Retrieves full event details including registration status, ticket, team info, and stock
...
// Registers participant for a normal event with form responses, file uploads, and QR ticket
export const registerForEvent = async (req, res) => {
    try {
        ...
        if (typeof formResponses === "string") {
            try { formResponses = JSON.parse(formResponses) } catch (e) { formResponses = {} }
        }

        if (req.files && req.files.length > 0) {
            for (const file of req.files) {
                const fieldId = file.fieldname.replace("file_", "")
                formResponses[fieldId] = await saveFile(file.buffer, file.originalname, file.mimetype, "registration")
            }
        }
...
// Processes merchandise purchase with stock validation, payment, and confirmation email
...
// Fetches ticket details including event info, QR code, and merchandise selections
...
// Cancels individual or team registration with stock restoration and member notifications
...
// Lists all organizers with follow status and optional fuzzy search
...
// Toggles follow/unfollow status for a specific organizer
...
// Fetches organizer profile with upcoming/past events and follow status
...
// Creates a team registration with member invitations, form responses, and email notifications
export const createTeam = async (req, res) => {
    try {
        ...
        const eventId = req.params.eventId
        let { teamName, memberEmails } = req.body || {}
        if (typeof memberEmails === "string") {
            try { memberEmails = JSON.parse(memberEmails) } catch (e) { memberEmails = [memberEmails] }
        }
...
// Retrieves pending team invitations for the current participant
...
// Handles accept/decline of team invitation; completes team and creates member registrations if all accepted
...
// Returns all teams where the participant is leader or accepted member
...
// Handles PDF file upload for team chat with membership and event status validation
export const uploadTeamChatFile = async (req, res) => {
    try {
        if (req.userData.role !== "Participant") {
            return res.status(403).json({ msgType: "Error", msg: "Permission denied." })
        }
        ...
        const fileUrl = await saveFile(req.file.buffer, req.file.originalname, req.file.mimetype, "chat")
...
// Returns unread message counts per team for the current participant
...
// Fetches team chat history with member details and chat availability status
...
// Generates an ICS calendar file for a single event with reminder alarms
...
export const exportCalendarBatch = async (req, res) => {
    try {
